function bound=interval(data_y,data_z,zbar,b,numRegressorsZ,maxStructuralChanges,robust,prewhit,hetomega,hetq,x,numRegressorsX)
...
if numRegressorsX==0
    ...
    res=data_y-zbar*delta;
else
    dbdel=olsqr(data_y,[zbar x]);
    res=data_y-[zbar x]*dbdel;
    delta=dbdel(1:(maxStructuralChanges+1)*numRegressorsZ,1);
end
bound=zeros(maxStructuralChanges,4);

bf=zeros(maxStructuralChanges+2,1);
bf(1,1)=0;
bf(2:maxStructuralChanges+1,1)=b(1:maxStructuralChanges,1);
bf(maxStructuralChanges+2,1)=nt;

for i=1:maxStructuralChanges
    delv=delta(i*numRegressorsZ+1:(i+1)*numRegressorsZ,1)-delta((i-1)*numRegressorsZ+1:i*numRegressorsZ,1);
    
    if robust==0
        if hetq==1
            
            qmat=data_z(bf(i,1)+1:bf(i+1,1),:)'*data_z(bf(i,1)+1:bf(i+1,1),:)...
                /(bf(i+1,1)-bf(i,1));
            qmat1=data_z(bf(i+1,1)+1:bf(i+2,1),:)'*data_z(bf(i+1,1)+1:bf(i+2,1),:)...
                /(bf(i+2,1)-bf(i+1,1));
            
        else
            qmat=data_z'*data_z/nt;
            qmat1=qmat;
        end
        ...
        if hetomega==1
            ...
            omega=correct(data_z(bf(i,1)+1:bf(i+1,1),:), res(bf(i,1)+1:bf(i+1,1),1),prewhit);
            omega1=correct(data_z(bf(i+1,1)+1:bf(i+2,1),:), res(bf(i+1,1)+1:bf(i+2,1),1),prewhit);
            
        else
            
            omega=correct(data_z,res,prewhit);
            omega1=omega;
