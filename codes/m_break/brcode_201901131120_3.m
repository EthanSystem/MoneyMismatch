...
% this is the new programs that modified by Ethan Lin start at 2019/01/13
...
% For any other commercial use and/or comments, please contactDelta_log_CNY Pierre
...
%% 
table_original_data=readtable('./data/预处理的数据_20190112.xlsx','Sheet','original');  % read data

% 需要计算的部分数据。
% 2008年7月至2015年7月，对应第103-187(end-1)号。
table_calculate_data=table_original_data(103:end-1,:);
% table_calculate_data=table_original_data;

numEffectiveSample=size(table_calculate_data,1)-1;                  % set effective sample size
numClassOfCurrency=5;                                      % %%%%%%%%%%%
...
Delta_log_CNY = calculate_DeltaAndLog_2(table_calculate_data.CNY);
Delta_log_USD = calculate_DeltaAndLog_2(table_calculate_data.USD);
Delta_log_JPY = calculate_DeltaAndLog_2(table_calculate_data.JPY);
Delta_log_KRW = calculate_DeltaAndLog_2(table_calculate_data.KRW);
Delta_log_EUR = calculate_DeltaAndLog_2(table_calculate_data.EUR);
Delta_log_GBP = calculate_DeltaAndLog_2(table_calculate_data.GBP);

Delta_EMP = calculate_EMP_3(table_calculate_data.CNY, table_calculate_data.ForeignExchangeReserve, table_calculate_data.ChinaBaseMoney);

data_y = Delta_log_CNY - Delta_log_GBP;
data_x = [Delta_log_USD - Delta_log_GBP, Delta_log_JPY - Delta_log_GBP, Delta_log_KRW - Delta_log_GBP, Delta_log_EUR - Delta_log_GBP, Delta_EMP];
data_z = 1.0 .*ones(numEffectiveSample,1);
...
minSizeOfSegment=10;       % minimal length of a segment (minSizeOfSegment >= numRegressorsZ). Note: if isHeterogeneityAndAutocorrelation=1, minSizeOfSegment 
...
maxIteration=100;                  % maximum number of iterations for the nonlinear procedure to 
