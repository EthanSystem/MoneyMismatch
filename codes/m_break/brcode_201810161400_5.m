...
table_data_original=readtable('./data/预处理的数据_20181126.xlsx','Sheet','original');  % read data
table_data_filtered = table_data_original(103:180,:);
numEffectiveSample=size(table_data_filtered,1)-1;                  % set effective sample size
...
Delta_log_CNY = calculate_DeltaAndLog_2(table_data_filtered.CNY);
Delta_log_USD = calculate_DeltaAndLog_2(table_data_filtered.USD);
Delta_log_JPY = calculate_DeltaAndLog_2(table_data_filtered.JPY);
Delta_log_KRW = calculate_DeltaAndLog_2(table_data_filtered.KRW);
Delta_log_EUR = calculate_DeltaAndLog_2(table_data_filtered.EUR);
Delta_log_GBP = calculate_DeltaAndLog_2(table_data_filtered.GBP);

Delta_EMP = calculate_EMP_3(table_data_filtered.CNY, table_data_filtered.ForeignExchangeReserve, table_data_filtered.ChinaBaseMoney);
...
minSizeOfSegment=11;       % minimal length of a segment (minSizeOfSegment >= numRegressorsZ). Note: if isHeterogeneityAndAutocorrelation=1, minSizeOfSegment 
...
maxIteration=26;                  % maximum number of iterations for the nonlinear procedure to 
