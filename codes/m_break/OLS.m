function [estimator,SE,SE_robust,sigma2,resid] = OLS(Y,X,add_constant)
...
if nargout == 0
    ...
    result(1,:)={'',' Estimator','SE','SE_robust','t-stat','numRegressorsX-val'};
    for maxStructuralChanges=1:nvar
           result(maxStructuralChanges+1,1)={['C(',num2str(maxStructuralChanges-add_constant),')']};
           result(maxStructuralChanges+1,2:end)={estimator(maxStructuralChanges),SE(maxStructuralChanges),SE_robust(maxStructuralChanges),t_stat(maxStructuralChanges),p_val(maxStructuralChanges)};
    end
    disp(' ')
